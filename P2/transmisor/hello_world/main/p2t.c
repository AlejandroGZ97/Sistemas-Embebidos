...
{    
    ...
    while(1)
    {
       ...
       // int len=0;
        ...
        //len=uart_read_bytes(UART2_PORT,cad_res,READ_BUF_SIZE,20/portTICK_RATE_MS);
        //cad_res[len]='\0';
       // uartPuts(0,cad_res);
       // uartGotoxy(0,2,1);
        ...
        uartPuts(0,"0x");
        uartGets(0,cad);  
        // printf("\n%d",packageUart.command);
        sendPacket(2,cad,0,0);

       // packageUart.command=myAtoi(cad);
        /* para recibir paquete  del receptor */
        /*receivePacket(2);
