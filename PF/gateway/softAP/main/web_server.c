...
{
    char command[5];
    size_t command_len = httpd_req_get_url_query_len(req) + 1;

    // Verificar si se recibió un comando válido en el cuerpo del mensaje POST
    if (command_len > 1 && command_len <= sizeof(command)) {
        // Leer el comando del cuerpo del mensaje
        httpd_req_get_url_query_str(req, command, command_len);
        command[command_len] = '\0';  // Agregar terminador nulo al final de la cadena
        ...
        strcpy(cad, "Comando inválido");
    }

    // Construir la respuesta HTTP con la variable "cad"
    httpd_resp_set_type(req, "text/plain");
    httpd_resp_set_hdr(req, "Access-Control-Allow-Origin", "*");
    httpd_resp_send(req, cad, strlen(cad));

    return ESP_OK;
}

static const httpd_uri_t CommandsPost = {
    .uri       = "/",
    .method    = HTTP_POST,
...
static httpd_handle_t start_webserver(void)
{
    ...
    if (httpd_start(&server, &config) == ESP_OK) {
        ...
        httpd_register_uri_handler(server, &CommandsPost);
        return server;
    }
